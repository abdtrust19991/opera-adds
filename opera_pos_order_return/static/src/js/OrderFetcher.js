odoo.define('point_of_sale.OrderFetcher', function (require) {
    ...
    class OrderFetcher extends EventBus {
        ...
        async _fetchOrders(ids) {
            ...
            var unpaid_orders = this.comp.env.pos.get_order_list() || [];
            var paid_orders = this.comp.env.pos.db.get_orders() || [];

            _.each(unpaid_orders, function(order){
                _.each(order.get_orderlines(), function(line){
                    if(line.order_line_id){
                        if(!lines_qty[line.order_line_id]){
                            lines_qty[line.order_line_id] = 0;
                        }
                        lines_qty[line.order_line_id] += Math.abs(line.get_quantity());
                    }
                })
            })

            _.each(paid_orders, function(order){
                _.each(order.lines, function(line){
                    var order_line_id = line[2].data.order_line_id ;
                    if( order_line_id ){
                        if(!lines_qty[order_line_id]){
                            lines_qty[order_line_id] = 0;
                        }
                        lines_qty[order_line_id] += Math.abs(line[2].data.qty);
                    }
            ...
            _.each(orders, function(order){
                _.each(order.lines, function(line){
                    if(line && line[2].id && lines_qty[line[2].id]){
                        line[2].return_qty += Math.abs(lines_qty[line[2].id]);
                    }
                })
            })
            return orders;
