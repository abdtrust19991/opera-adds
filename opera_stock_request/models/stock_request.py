...
class StockRequest(models.Model):
    ...
    available_route_ids = fields.Many2many(
        ...
        related='order_id.available_route_ids'
    )
    ...
    route_id = fields.Many2one(
        ...
        store=1,
        domain="[('stock_request_order_Lines', '=', True)]",
        states={"draft": [("readonly", False)]}, readonly=True)
    ...
    def _onchange_description(self):
        """ description """
        for rec in self:
            if rec.order_id.delivery_picking_id:
                rec.order_id.delivery_picking_id.update({
                    'partner_id': rec.route_id.rule_ids[1].location_id.partner_id.id
                })
                for l in rec.order_id.delivery_picking_id.move_ids_without_package:
                    l.update({
                        'description_picking': rec.description
                    })
            for line in rec.picking_ids:
                line.update({
                    'partner_id': rec.route_id.rule_ids[0].location_src_id.partner_id.id
                })
    ...
    @api.depends('order_id.global_route_id')
    def _compute_route_id(self):
        """ Compute route_id value """
        for rec in self:
            if rec.order_id.global_route_id:
                rec.route_id = rec.order_id.global_route_id
