...
class StockRequestOrder(models.Model):
    ...
    def _get_available_route_ids(self):
        if self.env.user.stock_location_route_ids:
            available_route_ids = self.env.user.stock_location_route_ids
        else:
            available_route_ids = self.env['stock.location.route'].search([('stock_request_order_Lines', '=', True)])
    ...
    location_id = fields.Many2one(
        ...
        "Location",
        readonly=True,
        domain=[('stock_request_location', '=', True),("usage", "in", ["internal", "transit"])],
        ondelete="cascade",
        required=True,
        states={"draft": [("readonly", False)]},
    )
    ...
    def _create_from_product_multiselect(self, products):
        ...
        action = self.env.ref("opera_stock_request.stock_request_order_action").read()[0]
        action["views"] = [
            (self.env.ref("opera_stock_request.stock_request_order_form").id, "form")
        ]
